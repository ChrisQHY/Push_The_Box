...
{
	if (map[p_pos.y][p_pos.x] != CELL_BOX_SHIFT && map[p_pos.y][p_pos.x] != CELL_BOX_COMPLITE) {
		return false;
	}
	switch (dir) {
	case MOVE_W:
		if (map[p_pos.y - 1][p_pos.x] == CELL_NULL || map[p_pos.y - 1][p_pos.x] == CELL_BOX_TARGET) {
			if (target_flag == true) {
	...
	case MOVE_A:
		if (map[p_pos.y][p_pos.x - 1] == CELL_NULL || map[p_pos.y][p_pos.x - 1] == CELL_BOX_TARGET) {
			if (target_flag == true) {
